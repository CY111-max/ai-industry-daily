...
- **定时**：GitHub Actions 每天 **09:00 / 21:00（北京时间）**抓取，也可在 Actions 页手动触发。
  GitHub 的排期会排队延迟 1~3 小时，所以每个批次提前埋三次兜底（早批 06:40/07:40/08:40，晚批 18:40/19:40/20:40），先跑成的那次落盘，后面几次发现无新增就不提交——保证 9 点打开页面就有当日内容。
...
- **历史**：每天 09:00 / 21:00 各一份**全量快照**（晚报 = 当天截至 21 点的全部内容，含早报全部条目）。顶栏 `‹` `›` 逐条切换，或点「历史」直接跳；保留最近 30 天
...
collector/
  ...
  dedupe.js                 # URL 归一化 + 批次文件扫描 + 近 7 天滚动去重
  ai.js                     # DeepSeek 摘要 / 简报调用（含 JSON 容错解析）
  config.js                 # 运行时配置（全部可用环境变量覆盖）
.github/workflows/daily.yml # 定时任务（每批次三次兜底）
data/                       # Actions 产出：YYYY-MM-DD-HHmm.json + index.json（自动提交）
serve.js                    # 本地预览用静态服务器
```
...
之后每天 **09:00 / 21:00（北京时间）**自动更新，不用再管。
...
node collector/run.js --date=2026-09-08 --slot=0900    # 指定日期与批次（0900 / 2100）
```
...
`data/YYYY-MM-DD-HHmm.json`（例 `2026-09-12-0900.json`；早期只有日期的 `YYYY-MM-DD.json` 仍兼容，页面显示为「全天」）：
...
- 本机（浏览器）会缓存最近 7 个批次的数据快照；勾选的关键词、自定义词也存本机，不上传。
- 页面**刷新按钮**的含义是「拉取最新已生成的数据」。真正的重新采集发生在 09:00 / 21:00 的定时任务，或在 Actions 页手动 Run workflow。
- GitHub Pages 有约 10 分钟 CDN 缓存，页面请求都带了时间戳参数，正常不会读到旧文件。
- 定时任务**实测会排队延迟 1.5~3.3 小时**（不是几分钟）。这正是每批次埋三次兜底、且批次由 cron 而非运行时刻决定的原因。
